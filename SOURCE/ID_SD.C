...
// K1n9_Duk3 mod: port is now a variable
#define	selreg(n)	outportb(alPort,n)
#define	writereg(n)	outportb(alPort+1,n)
#define	readstat()	inportb(alPort)
...
static	char			*ParmStrings[] =
						{
							"noal",
							"adlib",	// K1n9_Duk3 addition
							"oplport",
							nil
						};
*/
static	void			(*SoundUserHook)(void);
static	word			SoundNumber,SoundPriority,SoundNext=0;	// K1n9_Duk3 mod: added SoundNext
...
*/
static	word			/*far *sqHack,far *sqHackPtr,*/sqIndex,sqStart,sqHackLen,sqHackSeqLen;
static	long			sqHackTime;

// K1n9_Duk3 additions:
static	boolean			sqIsKMF;
static	word			musicrate = DEFAULT_MUSIC_RATE;
static	word			musicnum = LASTMUSIC;	// set to invalid music number by default

#define KMF_ID 0x1A464D4B

typedef struct {
	longword id;	// always KMF_ID
	word rate;		// playback rate in Hz (ignored in here!)
	word size;		// size of instruction data, in bytes
} KMFHeader;

#define musicsegs (audiosegs+STARTMUSIC)
...
#if 0	// K1n9_Duk3 mod: these don't work correctly on fast CPUs and we don't really need them
///////////////////////////////////////////////////////////////////////////
//
...
asm	mov		dx,alPort	// K1n9_Duk3 mod: use port variable instead of 0x388
asm	mov		al,[n]
asm	out		dx,al
...
SDL_ALService(void)
{
	...
	// stop music when music buffer was purged:
	if (!musicsegs[musicnum])
	{
		SD_MusicOff();
		return;
	}
	
	// don't update music while memory manager is moving music buffer around:
	if (mmsorting == (unsigned)musicsegs[musicnum])
	{
		alTimeCount++;
		return;
	}
	
	sqHackPtr = MK_FP((unsigned)musicsegs[musicnum], sqIndex);
	...
	if (sqIsKMF)
	{
		...
		{
			count = *sqHackPtr++;	// low byte is number of commands, high byte is delay after this block
			sqHackTime = alTimeCount + (count >> 8);
			count &= 0xFF;
			sqHackLen -= 2 + (count << 1);
			while (count--)
			{
				w = *sqHackPtr++;	// low byte is OPL register, high byte is value to write to that register
			asm	mov	dx,[w]
			asm	mov	[a],dl
			asm	mov	[v],dh
				alOut(a,v);
...
///////////////////////////////////////////////////////////////////////////
static boolean
SDL_DetectAdLib(boolean override)	// K1n9_Duk3 mod: added override parameter
{
...
// K1n9_Duk3 fix: SDL_Delay doesn't work on very fast CPUs (tested @ 533 MHz)
...
// end of fix
	...
	if ((((status1 & 0xe0) == 0x00) && ((status2 & 0xe0) == 0xc0)) || override)	// K1n9_Duk3 mod: added override check
	{
...
{
static	word	count = 1;
static	word	soundcount = 0;	// K1n9_Duk3 addition
#define SOUNDDIVISOR (1192030 / DEFAULT_SOUND_RATE)	// K1n9_Duk3 addition
...
{
	int	i;
	boolean forceAdLib = false;	// K1n9_Duk3 addition
	...
	for (i = 1;i < _argc;i++)
	{
		static stringnames ParmStrings[] = {S_NOAL, S_ADLIB, S_OPLPORT, -1};
		
		switch (SM_CheckParm(_argv[i],ParmStrings))
		{
		case 0:						// No AdLib detection
			alNoCheck = true;
			break;
		case 1:						// force AdLib detection (K1n9_Duk3 addition)
			forceAdLib = true;
			break;
		case 2:						// user-defined OPL port
			if (i+1 < _argc)
			{
				// parse hex number:
				char *s;
...
#if 0	// K1n9_Duk3 mod: don't need this anymore
	...
	if (!alNoCheck)
		AdLibPresent = SDL_DetectAdLib(forceAdLib);	// K1n9_Duk3 mod: added override parameter
...
SD_Default(boolean gotit,SDMode sd,SMMode sm)
{
	...
	if (gotsm)	// Make sure requested music hardware is available
	{
		switch (sm)
		{
		case smm_AdLib:	// K1n9_Duk3 fix: must use smm_AdLib here, not sdm_AdLib
			gotsm = AdLibPresent;
			break;
		}
	}
...
SD_PlaySound(soundnames sound)
{
	...
	if (!s)
		...
		SM_Quit(S_UNCACHEDSOUND);
	if (!s->length)
		//Quit("SD_PlaySound() - Zero length sound");
		SM_Quit(S_ZEROLENGTHSOUND);
	...
	SoundNext = 0;	// K1n9_Duk3 addition
}
...
///////////////////////////////////////////////////////////////////////////
//
//	SD_PlayTwoSounds() - plays two sound effects back to back
//
///////////////////////////////////////////////////////////////////////////
void
SD_PlayTwoSounds(soundnames sound1, soundnames sound2)
{
	SD_PlaySound(sound1);
	if (SD_SoundPlaying() == sound1)
	{
		SoundNext = sound2;
	}
}
...
SD_MusicOn(void)
{
	if (musicnum < LASTMUSIC && musicsegs[musicnum])
	{
		MM_SetPurge((memptr *)&musicsegs[musicnum], 0);	// just in case...
		sqActive = true;
	}
}
...
///////////////////////////////////////////////////////////////////////////
void
SD_StartMusic(unsigned num)
{
	MusicGroup _seg *music;
	
	SD_MusicOff();
asm	pushf
asm	cli

	if (MusicMode == smm_AdLib && num < LASTMUSIC && musicsegs[num])
	{
		musicnum = num;
		music = (MusicGroup _seg *)musicsegs[num];
		if (((KMFHeader _seg *)music)->id == KMF_ID)	// is KMF header?
		{
			sqIsKMF = true;
			sqIndex = sqStart = sizeof(KMFHeader);	// instruction data starts after KMF header
			sqHackSeqLen = sqHackLen = ((KMFHeader _seg *)music)->size;
		}
		else
		{
			sqIsKMF = false;
			sqIndex = sqStart = offsetof(MusicGroup, values);
...
SD_MusicPlaying(void)
{
	...
	{
	case smm_AdLib:
		result = sqActive;	// K1n9_Duk3 addition
		break;
	default:
		result = false;
	}
...
///////////////////////////////////////////////////////////////////////////
//
//	SD_SetMusicRate() - sets the playback rate for the music
//	(K1n9_Duk3 addition)
//
///////////////////////////////////////////////////////////////////////////
void
SD_SetMusicRate(word rate)
{
	if (rate < DEFAULT_SOUND_RATE)
		rate = DEFAULT_SOUND_RATE;
...
///////////////////////////////////////////////////////////////////////////
//
//	SD_CurrentMusic() - returns the current music number, i.e. the music that
// 	would be played/resumed when calling SD_MusicOn(), or -1 for no music
//	(K1n9_Duk3 addition)
//
///////////////////////////////////////////////////////////////////////////
int
SD_CurrentMusic(void)
{
	if (musicnum < LASTMUSIC && musicsegs[musicnum])
		return musicnum;
	return -1;
