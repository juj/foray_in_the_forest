...
// K1n9_Duk3 addition:
// FITF caches some sprite graphics on-the-fly and may change the current music
// at any time, which may cause the memory manager to perform a memory sort,
// which may move the mapsegs buffers around.
#define MAPSEGS_DONT_MOVE	false
// end of addition
...
#define MAXSCROLLEDGES		14	// Nisaba patch: Todo: is there actually a memory limit for this?
...
typedef	struct spriteliststruct
{
	...
	drawtype	draw;
	//unsigned	tilex,tiley,tilewide,tilehigh;	// K1n9_Duk3 mod: these were never actually used for anything
...
#ifndef KEEN
	//
	// Nisaba patch: Based on K1n9_Duk3's Keen 4-6 code, the game stores
	// the compatability setting in the game's config file.
...
#ifdef NEWMODS
		xpanmask = 7;	// DO pan to odd pixels!!!
#else
...
*/ //...replaced by K1n9_Duk3's Keen456 Code
...
//
	originxmin = originymin = MAPBORDER*TILEGLOBAL;	// K1n9_Duk3 addition
...
void RF_MarkTileGraphics (void)
{
	...
	unsigned	i,tilehigh;
	char		str[160],str2[10];
	byte		speed;	// K1n9_Duk3 addition
...
//
	...
	do
	{
		...
		if (tile>=0)			// <0 is a tile that is never drawn
		{
			...
			if (tinf[ANIM+tile])
			{
				// this tile will animate

				if ((speed=tinf[SPEED+tile]) != 0)	// K1n9_Duk3 addition: set speed
				{
#if 0
					// K1n9_Duk3 note: This check is useless in here. We are in the
					// "if (tinf[ANIM+tile])" branch, so the condition will always
					// evaluate to false in here and the code will never be executed.
					if (!tinf[ANIM+tile])
					{
						//strcpy (str,"RF_MarkTileGraphics: Background anim of 0:");
						SM_CopyString(str, S_BADBACKANIM);
						itoa (tile,str2,10);
						strcat (str,str2);
						Quit (str);
					}
				...
				while (change && next != tile)
				{
					...
					// K1n9_Duk3 addition: Animation chains that start with non-zero
					// SPEED values will cause an infinite loop in RFL_AnimateTiles()
					// if they lead to a non-animated tile.
					if (speed != 0 && change == 0 && tinf[SPEED+next] == 0)
					{
						//strcpy (str,"RF_MarkTileGraphics: Background anim of 0:");
						SM_CopyString(str, S_BADBACKANIM);
						itoa (next,str2,10);
					...
					if (++anims > 20)
					{
						...
						SM_CopyString(str, S_UNENDINGBACKANIM);
						itoa (next,str2,10);
						strcat (str,str2);
						Quit (str);
					}
	...
	do
	{
		...
		if (tile>=0)			// <0 is a tile that is never drawn
		{
			...
			if (tinf[MANIM+tile])
			{
				...
				if ((speed=tinf[MSPEED+tile]) != 0)	// K1n9_Duk3 addition: set speed
				{
#if 0
					// K1n9_Duk3 note: This check is useless in here. We are in the
					// "if (tinf[MANIM+tile])" branch, so the condition will always
					// evaluate to false in here and the code will never be executed.
					if (!tinf[MANIM+tile])
					{
						//strcpy (str,"RF_MarkTileGraphics: Foreground anim of 0:");
						SM_CopyString(str, S_BADFOREANIM);
						itoa (tile,str2,10);
						strcat (str,str2);
						Quit (str);
					}
				...
				while (change && next != tile)
				{
					...
					// K1n9_Duk3 addition: Animation chains that start with non-zero
					// MSPEED values will cause an infinite loop in RFL_AnimateTiles()
					// if they lead to a non-animated tile.
					if (speed != 0 && change == 0 && tinf[MSPEED+next] == 0)
					{
						//strcpy (str,"RF_MarkTileGraphics: Foreground anim of 0:");
						SM_CopyString(str, S_BADFOREANIM);
						itoa (next,str2,10);
					...
					if (++anims > 20)
					{
						...
						SM_CopyString(str, S_UNENDINGFOREANIM);
						itoa (next,str2,10);
						strcat (str,str2);
...
=========================
=
= RF_SaveState	(K1n9_Duk3 addition)
=
= Writes the origin and the used parts of allanims[] to a saved game file.
=
=========================
*/
/*
...
{
	if (write(handle, &originxglobal, sizeof(originxglobal)) == -1)
		return false;
	if (write(handle, &originyglobal, sizeof(originyglobal)) == -1)
		return false;
	if (write(handle, &numanimchains, sizeof(numanimchains)) == -1)
		return false;
	if (write(handle, &allanims, sizeof(allanims[0])*numanimchains) == -1)
		return false;
	return true;
}
*/
...
boolean RF_SaveState(int handle)
{
	if (write(handle, &originxglobal, sizeof(originxglobal)) != sizeof(originxglobal))
		return false;
	if (write(handle, &originyglobal, sizeof(originyglobal)) != sizeof(originyglobal))
		return false;
	if (write(handle, &numanimchains, sizeof(numanimchains)) != sizeof(numanimchains))
		return false;
	if (write(handle, &allanims, sizeof(allanims[0])*numanimchains) != sizeof(allanims[0])*numanimchains)
		return false;
	return true;
}
...
=========================
=
= RF_LoadState	(K1n9_Duk3 addition)
=
= Reads the origin and the tile animation states from a saved game file.
=
=========================
*/
...
{
	if (read(handle, &originxglobal, sizeof(originxglobal)) == -1)
		return false;
	if (read(handle, &originyglobal, sizeof(originyglobal)) == -1)
		return false;
	if (read(handle, &numanimchains, sizeof(numanimchains)) == -1)
		return false;
	if (read(handle, &allanims, sizeof(allanims[0])*numanimchains) == -1)
		return false;
	return true;
}
*/
...
boolean RF_LoadState(int handle)
{
	if (read(handle, &originxglobal, sizeof(originxglobal)) != sizeof(originxglobal))
		return false;
	if (read(handle, &originyglobal, sizeof(originyglobal)) != sizeof(originyglobal))
		return false;
	if (read(handle, &numanimchains, sizeof(numanimchains)) != sizeof(numanimchains))
		return false;
	if (read(handle, &allanims, sizeof(allanims[0])*numanimchains) != sizeof(allanims[0])*numanimchains)
		return false;
	return true;
}
...
#else
			// FITF version:
			mapsegs[current->plane][current->offset] = tile & 0x7fff; 		// change in map
#endif
...
void RF_SetScrollBlock (int x, int y, boolean horizontal)
{
	if (horizontal)
	{
		...
		if (hscrollblocks++ == MAXSCROLLEDGES)
			...
			SM_Quit(S_TOOMANYHSCROLLBLOCKS);
	}
	else
	{
		...
		if (vscrollblocks++ == MAXSCROLLEDGES)
			...
			SM_Quit(S_TOOMANYVSCROLLBLOCKS);
	}
}
...
=====================
=
= RF_PlaceSpriteAfter (K1n9_Duk3 addition)   EGA
=
= Add a sprite to the list, making sure it is added directly after the entry
= of the parent sprite, so that it will be drawn on top of the parent sprite. 
=
=====================
*/
...
{
	spritelisttype *sprite, *master, *next;
	RF_PlaceSprite(user, globalx, globaly, spritenumber, draw, priority);
	if (*user && *masteruser)
	{
		master = (spritelisttype *)*masteruser;
		sprite = (spritelisttype *)*user;
		if ((master->nextsprite != sprite))
		{
			// unlink current sprite
			next = sprite->nextsprite;
			if (next)						// if (!next), sprite is last in chain
				next->prevptr = sprite->prevptr;
			*sprite->prevptr = next;

			// link current sprite directly after master sprite
			sprite->prevptr = &master->nextsprite;
			next = master->nextsprite;
			sprite->nextsprite = next;
			if (next)
				next->prevptr = &sprite->nextsprite;
			master->nextsprite = sprite;
		}
	}
}
	...
	unsigned spritenumber, drawtype draw, int priority, int clipY)
{
	...
	spritelisttype oldspr;	// K1n9_Duk3 addition
...
remove:
	...
	// K1n9_Duk3 mod: check if clipping leaves anything visible
	spr = &spritetable[spritenumber-STARTSPRITES];
	if (abs(clipY) >= spr->height)
	{
		goto remove;
	}
	// end of mod
	...
	// sprite allready exists in the list, so we can use it's block
		oldspr = *sprite;	// K1n9_Duk3 addition: backup struct data into oldspr for later

		// K1n9_Duk3 mod: erase block handling has been moved to the end of this routine
	...
	// this is a brand new sprite, so allocate a block from the array
		...
		if (!spritefreeptr)
			...
			SM_Quit(S_NOFREESPRITESPOTS);

		sprite = spritefreeptr;
		spritefreeptr = spritefreeptr->nextsprite;

		memset(&oldspr, 0, sizeof(oldspr));	// K1n9_Duk3 addition: no random garbage in oldspr
...
//
	...
	if (!block)
	{
		...
		SM_CopyString(str, S_UNCACHEDSPRITE);
		itoa (spritenumber-STARTSPRITES,str2,10);	// K1n9_Duk3 mod: show sprite number, not chunk number
		strcat (str,str2);
		Quit (str);
	}

	if (!(draw & drawflipped))	// K1n9_Duk3 mod
		globaly+=spr->orgy;
	globalx+=spr->orgx;

	// K1n9_Duk3 mod: clipping
	if (clipY > 0)
		globaly += clipY << G_P_SHIFT;
	// end of mod
...
#define shift 0
	...
	sprite->height = spr->height - abs(clipY);	// K1n9_Duk3 mod: clipping
	sprite->grseg = spritenumber;
	sprite->sourceofs = block->sourceoffset[shift];
	// K1n9_Duk3 mod: clipping
	if (clipY > 0)
		sprite->sourceofs += spr->width * clipY;
	// end of mod
	...
	// K1n9_Duk3 mod: these values were written here but never used for anything
	/*
...
/*
#ifdef NEWMODS
	sprite->shift = pixx&7;		// how many pixels to shift rightwards when drawn
#endif
*/
#ifdef NEWMODS
#undef shift
...
// K1n9_Duk3 mod:
//	check if the new sprite (number, position, drawmode, clipping etc.) differs
//	from the old one and allow or skip redrawing the sprite based on that
//	(scorebox sprite must ALWAYS be updated!)
	if (spritenumber != SCOREBOXSPR && !memcmp(sprite, &oldspr, offsetof(spritelisttype, updatecount)))
	//if (spritenumber != SCOREBOXSPR && !memcmp(sprite, &oldspr, FP_OFF(&spritearray[0].updatecount)-FP_OFF(&spritearray[0])))
	{
		// exactly the same as before, so don't erase or update anything new:
		sprite->updatecount = oldspr.updatecount;
		return;
	}

	if (oldspr.grseg)	// grseg is never 0 for real sprite entries
	{
	//
	...
	//
		if (oldspr.updatecount<2)
		{
#ifdef NEWMODS
			// Make the erase block one byte bigger to the right
			if (oldspr.shift)	// K1n9_Duk3 mod: only increase width when shifts are used
				oldspr.width++;
#endif
			if (!oldspr.updatecount)
				memcpy (eraselistptr[otherpage]++,&oldspr,sizeof(eraseblocktype));
			memcpy (eraselistptr[screenpage]++,&oldspr,sizeof(eraseblocktype));
			
			// K1n9_Duk3 note: oldspr won't be used after this, so
			// we don't need to set its width back to the old value.
		}
	}
...
{
	RF_PlaceSpriteClipped(user, globalx, globaly, spritenumber, draw, priority, 0);
}
*/
...
//
	if (sprite->updatecount<2)
	{
#ifdef NEWMODS
		// Make the erase block one byte bigger to the right
		if (sprite->shift)	// K1n9_Duk3 mod: only increase width when shifts are used
			sprite->width++;
#endif
		...
		// K1n9_Duk3 note: the sprite entry won't be used after this,
		// so we don't need to set its width back to the old value.
	}
...
void RFL_UpdateSprites (void)
{
	...
	for (priority=0;priority<PRIORITIES;priority++)
	{
		...
		{
			boolean flipped = sprite->draw & drawflipped;	// K1n9_Duk3 mod
		//
...
#ifdef NEWMODS
			if (sprite->shift)
				xth = (portx + sprite->width) >> SX_T_SHIFT;	// add one screen byte in width for shifted sprites
			else
...
redraw:
		...
		//
			...
			{
				if (flipped)		// K1n9_Duk3 mod
					sourceofs += (porty+height-PORTSCREENHIGH)*sprite->width;
			...
			switch (sprite->draw & ~drawflipped)		// K1n9_Duk3 mod
			{
			case spritedraw:
#ifdef NEWMODS
				if (!sprite->shift && ((int)height) > 0)
				{
					VW_MaskBlock(grsegs[sprite->grseg], sourceofs,
						dest,sprite->width,height,sprite->planesize);
					break;
				}
				VW_ShiftlessMaskBlock(grsegs[sprite->grseg], sourceofs,
					dest,sprite->width,height,sprite->planesize, sprite->shift);
#else
				VW_MaskBlock(grsegs[sprite->grseg], sourceofs,
					dest,sprite->width,height,sprite->planesize);
#endif
			...
			// Addition for white-flashed sprites
#ifdef NEWMODS
			case whitedraw:
				VW_WhiteMaskBlock(grsegs[sprite->grseg], sourceofs,
					dest,sprite->width,height,WHITE, sprite->shift);		// K1n9_Duk3 mod
				break;

#if 0
			// K1n9_Duk3 addition: (this fading code was designed for Lindsey on LIGHTCYAN background)
			case fadedraw1:
				VW_WhiteMaskBlock(grsegs[sprite->grseg], sourceofs+4*sprite->planesize,
					dest,sprite->width,height,WHITE|0x100, sprite->shift);
			case fadedraw2:
				VW_WhiteMaskBlock(grsegs[sprite->grseg], sourceofs+sprite->planesize,
					dest,sprite->width,height,WHITE|0x100, sprite->shift);
			case fadedraw3:
				VW_WhiteMaskBlock(grsegs[sprite->grseg], sourceofs+2*sprite->planesize,
					dest,sprite->width,height,YELLOW|0x100, sprite->shift);
			case fadedraw4:
				VW_WhiteMaskBlock(grsegs[sprite->grseg], sourceofs+3*sprite->planesize,
					dest,sprite->width,height,LIGHTRED|0x100, sprite->shift);
#endif
#endif
...
unsigned RF_SpritesUsed(void)
{
	...
	for (i=0; i<PRIORITIES; i++)
	{
		...
		while (spr)
		{
			...
			spr = spr->nextsprite;
			if (count > MAXSPRITES)
				return count;	// just to avoid infinite loops
		}
	}
	return count;
}
...
{
	unsigned count = 0;
	spritelisttype *spr = spritefreeptr;
