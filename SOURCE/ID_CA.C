...
#define CAL_HuffExpand(source, dest, length, hufftable) _fmemcpy(dest, source, length)
...
//
	...
	if ((handle = open(GREXT"DICT."EXTENSION,
		...
		SM_QuitCannotOpen(GREXT"DICT."EXTENSION);
...
//
	...
	if ((handle = open(GREXT"HEAD."EXTENSION,
		...
		SM_QuitCannotOpen(GREXT"HEAD."EXTENSION);
...
convloop:
	...
	handle = open("foo.bin", O_CREAT|O_BINARY|O_WRONLY);
	if (handle != -1)
	{
		CA_FarWrite(handle, grstarts, (NUMCHUNKS+1)*FILEPOSSIZE);
		close(handle);
	}
	_CX = (NUMCHUNKS+1)*FILEPOSSIZE;
	asm {
...
convloop2:
	...
	handle = open("bar.bin", O_CREAT|O_BINARY|O_WRONLY);
	if (handle != -1)
	{
		CA_FarWrite(handle, grstarts, (NUMCHUNKS+1)*FILEPOSSIZE);
		close(handle);
...
//
	...
	if (grhandle == -1)
		...
		SM_QuitCannotOpen(GREXT"GRAPH."EXTENSION);
...
//
	...
	if ((handle = open("MAPHEAD."EXTENSION,
		...
		SM_QuitCannotOpen("MAPHEAD."EXTENSION);
...
//
	...
	if ((maphandle = open("GAMEMAPS."EXTENSION,
		...
		SM_QuitCannotOpen("GAMEMAPS."EXTENSION);
#else
	if ((maphandle = open("MAPTEMP."EXTENSION,
		 O_RDONLY | O_BINARY, S_IREAD)) == -1)
		//Quit ("Can't open MAPTEMP."EXTENSION"!");
		SM_QuitCannotOpen("MAPTEMP."EXTENSION);
#endif
}
...
//
// load audiodct.ext (huffman dictionary for audio files)
//

#ifndef NOHUFF
	if ((handle = open("AUDIODCT."EXTENSION,
		 O_RDONLY | O_BINARY, S_IREAD)) == -1)
		//Quit ("Can't open AUDIODCT."EXTENSION"!");
		SM_QuitCannotOpen("AUDIODCT."EXTENSION);

	read(handle, &audiohuffman, sizeof(audiohuffman));
	close(handle);
	CAL_OptimizeNodes (audiohuffman);
#endif
//
// load audio header and huffman dictionary
//
	if ((handle = open("AUDIOHED."EXTENSION,
		 O_RDONLY | O_BINARY, S_IREAD)) == -1)
		//Quit ("Can't open AUDIOHED."EXTENSION"!");
		SM_QuitCannotOpen("AUDIOHED."EXTENSION);
	length = filelength(handle);
...
//
	...
	if ((audiohandle = open("AUDIOT."EXTENSION,
		...
		SM_QuitCannotOpen("AUDIOT."EXTENSION);
#else
	if ((audiohandle = open("AUDIO."EXTENSION,
		 O_RDONLY | O_BINARY, S_IREAD)) == -1)
		//Quit ("Can't open AUDIO."EXTENSION"!");
		SM_QuitCannotOpen("AUDIO."EXTENSION);
#endif
}
...
#if 0	// these are never used in FITF
...
#if (GRMODE == EGAGR) && !(defined NEWMODS)
...
//
	spr = &spritetable[chunk-STARTSPRITES];
	smallplane = spr->width*spr->height;
	expanded = smallplane*5 + offsetof(spritetype, data);
...
// fake the shifts!
//
	dest->sourceoffset[0] = offsetof(spritetype, data);
	dest->planesize[0] = smallplane;
	dest->width[0] = spr->width;
#else
//
...
//
	...
	shiftstarts[0] = offsetof(spritetype, data);	// start data after 3 unsigned tables
...
#if 0	// CAL_ReadGrChunk is not used anywhere!
/*
======================
=
= CAL_ReadGrChunk
...
#if 0	// Keen Dreams style loading screens are never used in FITF
/*
======================
=
= CAL_DialogDraw
...
//
	...
	VWB_DrawTile8(thx,thy,11);		// Keen Dreams numbers
