...
#ifdef CKEEND
extern	boolean		walkingsounds;
#endif
#ifdef  KEEN
extern  boolean         oldshooting;
//extern  ScanCode        firescan;
extern  int             brainfucled;	// K1n9_Duk3 addition
extern  unsigned        bestscore;	// K1n9_Duk3 addition
extern  word            latchpel;	// K1n9_Duk3 addition

// K1n9_Duk3 addition:
ConfigType config = {
	// the initialization must be done in the order given in the ConfigType struct declaration (see ID_US.H)
	0,					// cpflags
	sc_Return,		// statusscan
	sc_RShift,		// detectorscan
	sc_BackSpace,	// scoreboxscan
	sc_F2,			// savemenuscan
	sc_F3,			// loadmenuscan
	sc_F5,			// quicksavescan
	sc_F9			// quickloadscan
...
static  boolean         US_Started;
		...
		// Keen Dreams import
		// K1n9_Duk3 fix: no initialization here!
...
#ifdef CKEEND
		read(file,&walkingsounds,sizeof(walkingsounds));
#endif
#ifdef KEEN
		read(file,&oldshooting,sizeof(oldshooting));
//		read(file,&firescan,sizeof(firescan));
		read(file,&brainfucled,sizeof(brainfucled));	// K1n9_Duk3 addition
		read(file,&bestscore,sizeof(bestscore));	// K1n9_Duk3 addition
		read(file,&hadAdLib,sizeof(hadAdLib));	// K1n9_Duk3 addition
		read(file,&mintics,sizeof(mintics));	// K1n9_Duk3 addition
		read(file,&latchpel,sizeof(latchpel));	// K1n9_Duk3 addition
		read(file,&config,sizeof(config)); // K1n9_Duk3 addition
#endif
		close(file);
...
USL_WriteConfig(void)
{
	...
	file = open("CONFIG."EXTENSION,O_CREAT | O_BINARY | O_WRONLY | O_TRUNC,	// K1n9_Duk3 mod: added O_TRUNC
...
#ifdef CKEEND
		write(file,&walkingsounds,sizeof(walkingsounds));
#endif
#ifdef KEEN
		write(file,&oldshooting,sizeof(oldshooting));
//		write(file,&firescan,sizeof(firescan));
		write(file,&brainfucled,sizeof(brainfucled));	// K1n9_Duk3 addition
		write(file,&bestscore,sizeof(bestscore));	// K1n9_Duk3 addition
		write(file,&AdLibPresent,sizeof(AdLibPresent));	// K1n9_Duk3 addition
		write(file,&mintics,sizeof(mintics));	// K1n9_Duk3 addition
		write(file,&latchpel,sizeof(latchpel));	// K1n9_Duk3 addition
		write(file,&config,sizeof(config)); // K1n9_Duk3 addition
#endif
		close(file);
	}
}
...
USL_CheckSavedGames(void)
{
	...
	for (i = 0,game = Games;i < MaxSaveGames;i++,game++)
	{
		...
		else
		{
			...
			game->present = false;
			game->timestamped = false;	// K1n9_Duk3 addition
			//strcpy(game->name,"Empty");
			SM_CopyString(game->name, S_EMPTY);
		}
	}
}
...
US_Startup(void)
{
	...
	*/
	...
	for (i = 1;i < _argc;i++)
	{
		static stringnames ParmStrings2[] = {S_COMP, S_NOCOMP, S_JERK, -1};
		
		switch (SM_CheckParm(_argv[i],ParmStrings2))
		{
		case 0:
			//if (grmode == EGAGR)	// is ALWAYS EGAGR in FITF
			compatability = true;
			break;
		case 1:
			compatability = false;
...
US_TextScreen(void)
{
	...
	*/
	...
	for (i = 1;i < _argc;i++)
	{
		// K1n9_Duk3 mod: using String Manager, TEDLEVEL also counts as NOWAIT
		static stringnames ParmStrings[] = {S_TEDLEVEL, S_NOWAIT, -1};
		...
		case 0:
			...
			if (tedlevelnum)
				...
				SM_Quit(S_BADTEDLEVEL);
		// no break here -- I want TEDLEVEL to count as NOWAIT, too!
#endif
		case 1:
			NoWait = true;
		}
...
US_UpdateTextScreen(void)
{
	...
	USL_ScreenDraw(27,22,SM_GetString(S_STARTUP_LOADING),0x9c);
}
...
static  byte    colors[] = {4,6,13,15,15,15,15,15,15};
	...
	ready = SM_GetString(S_STARTUP_READY);

	/*
	if (!(tedlevel || NoWait))
	*/
	if (!NoWait)	// K1n9_Duk3 mod (tedlevel also sets NoWait)
	{
		IN_ClearKeysDown();
		for (i = 0,up = true;!IN_UserInput(4,true);)
		{
			...
			USL_ScreenDraw(29,22,ready,0x00 + c);
		}
	}
	else
		//USL_ScreenDraw(29,22," Ready - Press a Key     ",0x9a);
		USL_ScreenDraw(29,22,ready,0x9a);
	IN_ClearKeysDown();
...
US_CPrintLine(char *s)
{
	...
	if (w > WindowW)
	{
		...
		str[h++] = '\n';
		str[h] = 0;
		SM_ConcatString(str, S_STRINGEXCEEDSWIDTH);
		Quit(str);
	}
...
US_DrawWindow(word x,word y,word w,word h)
{
	...
	WindowH = h * 8;
	/*
	// K1n9_Duk3 fix: these are redundant (will be set by US_ClearWindow)
	PrintX = WindowX;
	PrintY = WindowY;
	*/
	sx = (x - 1) * 8;
				...
				int maxchars,int maxwidth)
{
	...
	*olds = '\0';
	...
	asm     popf
		...
		{
			len = strlen(s);
			// K1n9_Duk3 addition: temporarily add the new char to get the total width
			if (len < MaxString - 1)
			{
				s[len] = c;
				s[len+1] = '\0';
			...
			// end of addition
			USL_MeasureString(s,&w,&h);
			s[len] = '\0';	// K1n9_Duk3 addition (remove the char again)
