 ...
 *
 * Primarily based on:
 * Catacomb 3-D Source Code * Copyright (C) 1993-2014 Flat Rock Software
 *
...
/*
static	char *ParmStrings[] = {"HIDDENCARD","NOBORDER", ""};	// K1n9_Duk3 fix: added JERK, Nisaba patch: replaced JERK with NOBORDER

void	VW_Startup (void)
{
	extern word latchpel;	// K1n9_Duk3 fix
	int i;
	...
	for (i = 1;i < _argc;i++)
		...
		// K1n9_Duk3 fix:
		case 1:
		//	Nisaba patch: replaced 'latchpel = 0;' with 'bordercolor = 0xFF00;'
		//	latchpel = 0; 
			bordercolor = 0xFF00;
			break;
		}
...
// Nisaba patch:
static stringnames ParmStrings[] = {S_HIDDENCARD, S_NOBORDER, S_VGA, -1};
...
#ifdef NEWMODS
	VW_ShiftlessMaskBlock(block,block->sourceoffset[0],dest,
		block->width[0],spr->height,block->planesize[0], shift);
#else
...
// Setting the CRTC start address is not found in the Keen 5 Disassembly
// 
#ifndef NEWMODS
asm	cli
...
asm	mov	dx,CRTC_INDEX
asm	mov	al,CRTC_STARTHIGH  // start address high register
asm	out	dx,al
asm	inc	dx
asm	mov	al,ch
asm	out	dx,al
asm	dec	dx
asm	mov	al,CRTC_STARTLOW  // start address low register
asm	out	dx,al
asm	mov	al,cl
asm	inc	dx
asm	out	dx,al
...
#ifdef NEWMODS
		VW_ShiftlessMaskBlock(block,block->sourceoffset[0],dest,
			width,height,block->planesize[0], (x&7));
#else
		VW_MaskBlock (block,block->sourceoffset[shift],dest,
